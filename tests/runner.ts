...
export async function runAllTests(db: Database): Promise<TestResults> {
  const results: Partial<TestResults> = {}

  for (const cap of capabilities) {
    try {
      results[cap.id as CapabilityId] = await cap.test(db)
    }
    catch (error) {
...
export async function runTest(db: Database, capabilityId: CapabilityId): Promise<CapabilityResult> {
